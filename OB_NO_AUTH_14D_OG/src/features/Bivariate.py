...
def bivar_exec(src="", wrk="", oup="", indsn="", resp="", timelwlmt="",
               timeuplmt=""):
    """
    ...
    if src == "" or wrk == "" or oup == "" or indsn == "" or resp == "" or timelwlmt == "" or timeuplmt == "":
    ...
    req_list = clm_dtls[clm_dtls.BIVAR_TYPE.isin(["NOMINAL", "ORDINAL", "BINARY"])]['VAR_NAME']
    # req_list = bin_df_train.columns.drop(['target_date_12', 'patient_id', resp])
    len(list(req_list))
    ...
    for x in req_list:
        ...
        bin_bi_chisq = pd.concat([bin_bi_chisq, temp_df[['VAR_NAME', 'CHI-SQUARE_VAL', 'P-VALUE', 'DOF']]],
                                 ignore_index=True)

    bin_bi_fisher = pd.DataFrame(columns=['VAR_NAME', 'P-VALUE'])
    binary_list = clm_dtls[clm_dtls.BIVAR_TYPE.isin(["BINARY"])]['VAR_NAME']
    for x in binary_list:
        # Perform Fisher's exact test on the contingency table
        c_table = pd.crosstab(bin_df_train[x], bin_df_train[resp])
        p_value = fisher_exact(c_table)[1]
        # Add the results to the DataFrame
        bin_bi_fisher = pd.concat([bin_bi_fisher, pd.DataFrame({'VAR_NAME': [x], 'P-VALUE': [p_value]})],
                                  ignore_index=True)
    # Mutual information
    print(" MI LOGIC")
    ...
    for index, row in clm_dtls.iterrows():
        ...
        elif (BIVAR_type == 'ORDINAL' and DEF_type == 'NUM'):
            # Ordinal variable
        ...
        elif (BIVAR_type == 'NOMINAL' and DEF_type == 'CHAR'):
    ...
    Clmn_list = clm_dtls[clm_dtls.BIVAR_TYPE.isin(["NOMINAL", "ORDINAL", "BINARY", "NUM"])]['VAR_NAME']
